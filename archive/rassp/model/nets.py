...
# Function overview: goodmax handles a specific workflow step in this module.
def goodmax(x, dim):
    return torch.max(x, dim=dim)[0]
 
# Function overview: peak_indices_intensities_to_sparse_matrix handles a specific workflow step in this module.
...
# Class overview: MaskedBatchNorm1d encapsulates a reusable component in this module.
...
# Class overview: MaskedLayerNorm1d encapsulates a reusable component in this module.
...
# Class overview: GraphInstanceNorm encapsulates a reusable component in this module.
...
# Class overview: GraphMatLayer encapsulates a reusable component in this module.
class GraphMatLayer(nn.Module):
    ...
    def forward(self, G, x):
        # Function overview: apply_ll handles a specific workflow step in this module.
        def apply_ll(i, x):
...
# Class overview: GraphMatLayers encapsulates a reusable component in this module.
...
# Class overview: GraphMatLayersNormAfterRes encapsulates a reusable component in this module.
...
# Class overview: GraphMatHighwayLayers encapsulates a reusable component in this module.
...
# Function overview: parse_agg_func handles a specific workflow step in this module.
...
# Function overview: log_normal_nolog handles a specific workflow step in this module.
def log_normal_nolog(y, mu, std):
    element_wise =  -(y - mu)**2 / (2*std**2)  - std
    return element_wise

# Function overview: log_student_t handles a specific workflow step in this module.
def log_student_t(y, mu, std, v=1.0):
    return -torch.log(1.0 + (y-mu)**2/(v * std)) - std

# Function overview: log_normal handles a specific workflow step in this module.
def log_normal(y, mu, std):
    element_wise =  -(y - mu)**2 / (2*std**2)  - torch.log(std)
    return element_wise


# Class overview: GraphMatLayerFast encapsulates a reusable component in this module.
class GraphMatLayerFast(nn.Module):
    ...
    def forward(self, G, x):
        ...
        # ensure multi_x has shape (CHAN_N, BATCH_N, MAX_N, P)
        if multi_x.dim() > 4:
            # squeeze any singleton dimensions until 4D
            while multi_x.dim() > 4:
                squeezed = False
                for ax in range(multi_x.dim()):
                    if multi_x.shape[ax] == 1 and multi_x.dim() > 4:
                        multi_x = multi_x.squeeze(ax)
                        squeezed = True
                        break
                if not squeezed:
                    break

        # try to permute axes if sizes are reorderd
        BATCH_N, CHAN_N,  MAX_N, _ = G.shape
        if multi_x.dim() == 4:
            dims = list(multi_x.shape)
            # attempt to locate axes by matching sizes (heuristic)
            mapping = {}
            for i, s in enumerate(dims):
                if s == CHAN_N and 'chan' not in mapping:
                    mapping['chan'] = i
                elif s == BATCH_N and 'batch' not in mapping:
                    mapping['batch'] = i
                elif s == MAX_N and 'maxn' not in mapping:
                    mapping['maxn'] = i

            if set(mapping.keys()) == {'chan', 'batch', 'maxn'}:
                feat_idx = [i for i in range(4) if i not in mapping.values()][0]
                perm = [mapping['chan'], mapping['batch'], mapping['maxn'], feat_idx]
                if perm != [0,1,2,3]:
                    multi_x = multi_x.permute(perm).contiguous()

        if multi_x.dim() != 4:
            raise RuntimeError(f"unexpected multi_x dims {multi_x.shape} in GraphMatLayerFast")

        # compute per-channel outputs then aggregate across channels
        xout_c = torch.einsum("bcij,cbjm->cbim", [G, multi_x])
        if self.agg_func is not None:
            xout = self.agg_func(xout_c, dim=0)
        else:
            xout = xout_c.sum(dim=0)
        if self.dropout > 0:
...
# Class overview: GraphMatLayerFastPow encapsulates a reusable component in this module.
class GraphMatLayerFastPow(nn.Module):
    ...
    def forward(self, G, x):
        ...
        multi_x = torch.stack([apply_ll(i,x) for i in range(self.GS)], 0)
        # ensure multi_x is 4D (CHAN, BATCH, MAX_N, P)
        if multi_x.dim() > 4:
            while multi_x.dim() > 4:
                squeezed = False
                for ax in range(multi_x.dim()):
        ...
        if multi_x.dim() != 4:
            raise RuntimeError(f"unexpected multi_x dims {multi_x.shape} in GraphMatLayerFastPow")

        xout_c = torch.einsum("bcij,cbjm->cbim", [Gprod, multi_x])
        if self.agg_func is not None:
            xout = self.agg_func(xout_c, dim=0)
        else:
            xout = xout_c.sum(dim=0)
...
# Class overview: GraphMatLayerFastPowSwap encapsulates a reusable component in this module.
...
# Class overview: GraphMatLayerFastPowSingleLayer encapsulates a reusable component in this module.
...
# Function overview: apply_masked_1d_norm handles a specific workflow step in this module.
...
# Class overview: GraphMatLayerFastPow2 encapsulates a reusable component in this module.
...
# Function overview: create_nonlin handles a specific workflow step in this module.
...
# Class overview: MLP encapsulates a reusable component in this module.
class MLP(nn.Module):
    # Function overview: __init__ handles a specific workflow step in this module.
...
# Function overview: render_gaussian_batch_points handles a specific workflow step in this module.
...
# Function overview: dense_to_sparse handles a specific workflow step in this module.
...
# Class overview: GraphMatLayerFast2 encapsulates a reusable component in this module.
...
# Class overview: GraphMatLayerFast3 encapsulates a reusable component in this module.
...
# Class overview: GraphMatLayersInstanceNorm encapsulates a reusable component in this module.
