...
def train_mssubsetnet():
    ...
    selector_only_warmup_epochs = max(0, int(os.environ.get('SELECTOR_ONLY_WARMUP_EPOCHS', '3')))
        ...
        with torch.no_grad():
            for step, raw_batch in enumerate(tqdm(val_dl, desc=f'Epoch {epoch+1}/{epochs} [Val]', leave=False), start=1):
                ...
                with autocast(enabled=amp_enabled, dtype=amp_dtype):
                    ...
                    if os.environ.get("EVAL_USE_TEACHER_TOPK", "0") == "1" and torch.is_tensor(teacher_topk_mask):
                        model_topk_mask = teacher_topk_mask
                    else:
                        topk_candidate_mask_val = None
                        if os.environ.get("MODEL_TOPK_USE_ACTIVE_MASK", "0") == "1":
