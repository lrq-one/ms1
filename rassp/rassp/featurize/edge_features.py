...
# Function overview: feat_edges handles a specific workflow step in this module.
...
# Function overview: feat_edges_for_bipartite_model handles a specific workflow step in this module.
def feat_edges_for_bipartite_model(mol, adj, MAX_ATOM_N=48, MAX_EDGE_N=64):
    ...
    for bi in range(n_bonds):
        ...
        neighbor_inds = list(np.where(shortest_paths[ai] == 2)[0]) + list(np.where(shortest_paths[aj] == 2)[0])
        neighbor_inds = sorted(set(neighbor_inds) - {ai, aj})
        neighbor_atom_nums = [mol.GetAtomWithIdx(int(i)).GetAtomicNum() for i in neighbor_inds]
