...
from fragnnet.utils.misc_utils import scatter_logsoftmax, scatter_l1normalize, scatter_l2normalize, scatter_reduce, EPS, th_setdiff1d, safelog, scatter_logsumexp, TOLERANCE_MIN_MZ

# rough approximations from NIST20 Orbitrap/FT MS
# NCE_MEAN = 60.
# NCE_STD = 40.
# NCE_MAX = 200.
# rough approximations from NIST20 QToF
# ACE_MEAN = 20
# ACE_STD = 10
# ACE_MAX = 60.
MZ_MAX = 1500.0
...
def transform_ce(ce, ce_mean, ce_std):
	return (ce-ce_mean)/ce_std
...
def batched_bin_func(
		...
		prec_mzs:th.Tensor=None,
		return_mzs:bool=False) -> th.Tensor:
	...
	if sparse:
		...
		if return_mzs:
			un_bin_mzs = (bins[un_bin_idxs % num_bins] - mz_bin_res/2.)
...
def calculate_match_mzs(
	...
	if relative:
		if pred_mz_divisor:
			divisor_mzs = copy_func(pred_mzs)
		else:
...
def calculate_sparse_cosine_similarity(
	...
	if log_ints:
		true_bin_ints = true_bin_ints.exp()
		pred_bin_ints = pred_bin_ints.exp()
...
def calculate_sparse_cosine_hungarian(
	true_mzs:th.Tensor, 
	true_ints:th.Tensor,
	true_batch_idxs:th.Tensor,
	pred_mzs:th.Tensor,
	...
	""" Calculate cosine similarity using the Hungarian algorithm as the matching method.
	...
	Args:
		...
		pred_batch_idxs (th.Tensor): 1d flat tensor of predicted batch indices, each cell, pred_batch_idxs[i] indicates batch index for pred_mzs[i] and pred_ints[i], should be the same size as mz tensor
		tolerance (float):
		relative (bool, optional): flag to use relative match else use abs tolerance measured in Da. Defaults to True.
		sum_ints (bool, optional): flag to get summed intesnities in aggregate, else get max. Defaults to True.

	Returns:
		th.Tensor: cosine similarity tensor, one per pair 
	"""
	# round peaks
	agg = "sum" if sum_ints else "amax"
	...
	# calculate
	batch_size = th.max(true_batch_idxs_r)+1
	cos_hun = th.zeros([batch_size],device=true_ints.device,dtype=true_ints.dtype)
	for b_idx in range(batch_size):
		...
		b_true_prec_mask_r = None
		b_pred_prec_mask_r = None
		#assert remove_prec_peaks and true_prec_mzs is None, 
		if remove_prec_peaks:
			# precursor
...
def batched_l2_normalize(ints, batch_idxs):

	ints = scatter_l2normalize(
		ints,
		batch_idxs
	)
	return ints
...
def mf1000_normalize_np(ints: np.array) -> np.array:
	""" mf 1000 normalize implemetion in numpy, not batched

	Args:
		ints (np.array): peak intensity 

	Returns:
		np.array: normalized intensities
	"""
	int_max = np.max(ints)
	ints = (ints / int_max) * 1000.
	return ints
...
def jss_hun_helper(
	...
	b_kl1 = th.sum(b_kl1_true_probs*(safelog(b_kl1_true_probs,eps=log_min)-safelog(b_kl1_union_probs,eps=log_min)), dim=0)
	...
	b_kl2 = th.sum(b_kl2_pred_probs*(safelog(b_kl2_pred_probs,eps=log_min)-safelog(b_kl2_union_probs,eps=log_min)), dim=0)
	b_jss_hun = np.log(2.)-0.5*(b_kl1+b_kl2)
	return b_jss_hun
...
def jss_helper(
	...
	kl1 = scatter_reduce(
		true_bin_ints * (safelog(true_bin_ints,eps=log_min) - safelog(kl1_union_bin_ints,eps=log_min)),
		true_bin_batch_idxs,
		reduce="sum",
		dim_size=batch_size
	)
	...
	kl2 = scatter_reduce(
		pred_bin_ints * (safelog(pred_bin_ints,eps=log_min) - safelog(kl2_union_bin_ints,eps=log_min)),
		pred_bin_batch_idxs,
		reduce="sum",
		dim_size=batch_size
	)
