...
class MolAttentionGRUNewSparse(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # but rendering must happen at formula-group level.
        # Otherwise one formula with many source instances can occupy
        # too much probability mass and collapse support diversity.
        # ------------------------------------------------------------
        if os.environ.get("USE_GROUP_AWARE_RENDER_PROBS", "0") == "1":
            ...
            if torch.is_tensor(formulae_probs_group_render):
                formulae_probs_render = formulae_probs_group_render
            else:
